"""Builtin owner entry for the WidarGait dataset definition."""
...
def _resolve_task_binding_contract(binding_id: str | None) -> dict[str, object]:
    ...
    if not candidate:
        ...
        raise ValueError(
            "WidarGait requires an explicit task_binding; "
    ...
    if candidate not in available:
        supported = ", ".join(available) or "<none>"
        raise ValueError(
            f"WidarGait task_binding must be one of: {supported}. Received {candidate!r}."
        )
...
def _artifact_materialization(artifact: DatasetBuildArtifact) -> dict[str, object]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("WidarGait artifact materialization payload must be a mapping.")
    return dict(payload)
...
def _resolve_materialized_view_contract(
    ...
    task_id, target_schema = resolve_materialized_task_identity(
        materialization,
        owner="WidarGait artifact materialization payload",
    )
...
def build(request: DatasetBuildRequest) -> DatasetBuildArtifact:
    ...
    if supported_variants and manifest_plan.variant not in supported_variants:
        raise ValueError(
            f"WidarGait task binding '{task_binding['binding_id']}' does not support variant "
            f"{manifest_plan.variant!r}. Supported variants: {', '.join(supported_variants)}."
        )
    binding_payload = resolve_dataset_task_binding_payload(task_binding, owner="WidarGait")
    materialized_target_schema = _materialized_target_schema(
        binding_payload,
        task_binding=task_binding,
        owner=f"WidarGait task binding '{task_binding['binding_id']}'",
    )
...
def materialize_views_from_artifact(
    ...
    source_dataset = load_widargait_dataset(str(resolved_path), variant=variant)
