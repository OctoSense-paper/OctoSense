"""Builtin owner entry for the XRFV2 dataset definition."""
...
from .manifest import plan_xrfv2_manifest_build
...
def _resolve_variant_payload(
    ...
    *,
    ...
    if not candidate:
        ...
        raise ValueError(
            "XRFV2 requires an explicit config; "
    ...
    if not matches:
        ...
        if modality is None:
            raise ValueError(
                f"XRFV2 variant must resolve from canonical variant '{candidate}' and an explicit modality. "
                f"Supported config bindings: {supported}."
            )
        raise ValueError(
            f"XRFV2 variant '{candidate}' does not support modality {modality!r}. "
            f"Supported config bindings: {supported}."
        )
    if len(matches) > 1:
        ...
        raise ValueError(
            f"XRFV2 variant '{candidate}' is shared by multiple modalities ({matching_modalities}). "
    ...
    if variant_key != binding_id:
        raise ValueError(
            f"XRFV2 config binding '{binding_id}' declares mismatched variant_key {variant_key!r}."
        )
...
def _materialization_payload(artifact: DatasetBuildArtifact) -> dict[str, object]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("XRFV2 artifact materialization payload must be a mapping.")
    return payload
...
def _resolve_task_binding_contract(binding_id: str | None) -> dict[str, Any]:
    ...
    if not candidate:
        supported = ", ".join(available) or "<none>"
        raise ValueError(
            "XRFV2 requires an explicit task_binding; "
            "implicit default/singleton fallback is not supported. "
            f"Supported bindings: {supported}."
        )
    if candidate not in available:
        supported = ", ".join(available) or "<none>"
        raise ValueError(
            f"XRFV2 task_binding must be one of: {supported}. Received {candidate!r}."
        )
...
def build(request: DatasetBuildRequest) -> DatasetBuildArtifact:
    ...
    binding_payload = resolve_dataset_task_binding_payload(task_binding, owner="XRFV2")
    dataset_target_schema = _dataset_target_schema(
        binding_payload,
        owner="XRFV2 task binding compiler payload",
    )
    manifest_plan = plan_xrfv2_manifest_build(
        request,
...
def materialize_views_from_artifact(
    ...
    if not resolved_variant:
        raise ValueError("XRFV2 artifact is missing a canonical variant.")
    config_binding_id = str(materialization.get("config_binding_id", "")).strip()
    if not config_binding_id:
        raise ValueError("XRFV2 artifact materialization payload is missing config_binding_id.")
    resolved_modality = str(materialization.get("modality", "")).strip()
    if not resolved_modality:
        raise ValueError("XRFV2 artifact materialization payload is missing modality.")
    task_id, task_kind, target_kind, target_schema = _resolve_artifact_task_contract(
        materialization,
        owner="XRFV2 artifact materialization payload",
    )
    ...
    test_dataset = XRFV2Dataset(
        resolved_root,
        modality=resolved_modality,
        variant=config_binding_id,
        split="test",
    )
    label_mapping = train_dataset.get_label_mapping() if _requires_label_mapping(target_kind) else None
    ...
    for split_name in ("train", "val", "test"):
        ...
        views_by_split[split_name] = build_manifest_backed_view(
            dataset=split_sources[split_name],
    ...
    if not views_by_split:
        raise ValueError("XRFV2 artifact materialization produced no DatasetView splits.")
...
def build_view(
    ...
    *,
    ...
    materialization = _materialization_payload(artifact)
    task_id, _, _, _ = _resolve_artifact_task_contract(
        materialization,
        owner="XRFV2 artifact materialization payload",
    )
    (
