"""Builtin owner entry for the FallDar dataset definition."""
...
def _resolve_task_binding_contract(binding_id: str | None) -> dict[str, object]:
    ...
    if not candidate:
        ...
        raise ValueError(
            "FallDar requires an explicit task_binding; "
    ...
    if candidate not in available:
        supported = ", ".join(available) or "<none>"
        raise ValueError(
            f"FallDar task_binding must be one of: {supported}. Received {candidate!r}."
        )
...
def _artifact_materialization(artifact: DatasetBuildArtifact) -> dict[str, object]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("FallDar artifact materialization payload must be a mapping.")
    return dict(payload)
...
def _resolve_materialized_view_contract(
    ...
    task_id, target_schema = resolve_materialized_task_identity(
        materialization,
        owner="FallDar artifact materialization payload",
    )
...
def build(request: DatasetBuildRequest) -> DatasetBuildArtifact:
    ...
    if supported_variants and variant not in supported_variants:
        raise ValueError(
            f"FallDar task binding '{task_binding['binding_id']}' does not support variant "
            f"{variant!r}. Supported variants: {', '.join(supported_variants)}."
        )
    binding_payload = resolve_dataset_task_binding_payload(task_binding, owner="FallDar")
    materialized_target_schema = _materialized_target_schema(
        binding_payload,
        task_binding=task_binding,
        owner=f"FallDar task binding '{task_binding['binding_id']}'",
    )
...
def materialize_views_from_artifact(
    ...
    variant = _resolve_variant(getattr(artifact, "variant", None))
    source_dataset = load_falldar_dataset(resolved_path, variant=variant)
